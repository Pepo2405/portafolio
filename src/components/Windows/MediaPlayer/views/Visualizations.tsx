...
export default function Visualizations({ viz, onVizChange }: Props) {
  const t = useT();
  return (
    <div className="wmp-scroll h-full overflow-y-auto p-4 pb-8 text-white">
      <div className="mb-3 border-b border-white/15 pb-1 text-[11px] font-bold uppercase tracking-wide text-white/60">
        {t("wmp.visualizations")}
      </div>
      <div className="flex flex-col gap-2" role="listbox" aria-label={t("wmp.chooseViz")}>
        {VIZ_LIST.map((v) => {
